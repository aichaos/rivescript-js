...
return undefined;};RiveScript.prototype.reply=function(user,msg,scope){this.say("Asked to reply to ["+user+"] "+msg);msg=this._format_message(msg);var reply='';if(this._topics["__begin__"]&&false){var begin=this._getreply(user,"request","begin",0,scope);if(begin.indexOf("{ok}")>-1){reply=this._getreply(user,msg,"normal",0,scope);begin=begin.replace(/\{ok\}/g,reply);}
reply=begin;}else{reply=this._getreply(user,msg,"normal",0,scope);}
