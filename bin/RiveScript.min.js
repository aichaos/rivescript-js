...
+code.join("\n")
+"}\n";try{eval(source);}catch(e){this._master.warn("Error evaluating JavaScript object: "+e.message);}};JsRiveObjects.prototype.call=function(rs,name,fields){var func=this._objects[name];var reply="";try{reply=func.call(undefined,rs,fields);}catch(e){reply="[ERR: Error when executing JavaScript object]";}
return reply;};var VERSION="1.00";var RS_VERSION="2.0";function RiveScript(opts){this._debug=false;this._strict=true;this._depth=50;this._div=undefined;this._node={};this._runtime=this.runtime();this._pending=[];this._loadcount=0;this._gvars={};this._bvars={};this._subs={};this._person={};this._arrays={};this._users={};this._freeze={};this._includes={};this._lineage={};this._handlers={};this._objlangs={};this._topics={};this._thats={};this._sorted={};if(typeof(opts)=="object"){if(opts["debug"]){this._debug=opts["debug"]?true:false;}
...
RiveScript.prototype.runtime=function(){if(typeof(window)=="undefined"&&typeof(module)=="object"){this._node["fs"]=require("fs");return"node";}else{return"web";}};RiveScript.prototype.say=function(message){if(this._debug!=true){return;}
if(this._div){$(this._div).append("<div>[RS] "+this._escape_html(message)+"</div>");}else if(console){console.log("[RS] "+message);}};RiveScript.prototype.warn=function(message,fname,lineno){if(typeof(fname)!="undefined"&&typeof(lineno)!="undefined"){message+=" at "+fname+" line "+lineno;}
if(this._div){$(this._div).append("<div style='color: #FF0000; font-weight: bold'>"
+this._escape_html(message)+"</div>");}else if(console){if(console.error){console.error(message);}else{console.log("[WARNING] "+message);}}else{window.alert(message);}}
...
return undefined;};RiveScript.prototype.reply=function(user,msg){this.say("Asked to reply to ["+user+"] "+msg);msg=this._format_message(msg);var reply='';if(this._topics["__begin__"]&&false){var begin=this._getreply(user,"request","begin",0);if(begin.indexOf("{ok}")>-1){reply=this._getreply(user,msg,"normal",0);begin=begin.replace(/\{ok\}/g,reply);}
reply=begin;}else{reply=this._getreply(user,msg,"normal",0);}
this._users[user]["__history__"]["input"].pop();this._users[user]["__history__"]["input"].unshift(msg);this._users[user]["__history__"]["reply"].pop();this._users[user]["__history__"]["reply"].unshift(reply);return reply;};RiveScript.prototype._format_message=function(msg){msg=""+msg;msg=msg.toLowerCase();msg=this._substitute(msg,"subs");msg=this._strip_nasties(msg);return msg;};RiveScript.prototype._getreply=function(user,msg,context,step){if(!this._sorted["topics"]){this.warn("You forgot to call sortReplies()!");return"ERR: Replies Not Sorted";}
...
var name=match[1];var value=match[2];this._users[user][name]=value;}}else{reply=this._process_tags(user,msg,reply,stars,thatstars,step);}
...
return regexp;};RiveScript.prototype._process_tags=function(user,msg,reply,st,bst,step){var stars=[""];stars.push.apply(stars,st);var botstars=[""];botstars.push.apply(botstars,bst);if(stars.length==1){stars.push("undefined");}
...
var target=this._strip(match[1]);this.say("Inline redirection to: "+target);var subreply=this._getreply(user,target,"normal",step+1);reply=reply.replace(new RegExp("\\{@"+this.quotemeta(target)+"\\}","i"),subreply);match=reply.match(/\{@(.+?)\}/);}
...
var output="";if(this._objlangs[obj]){var lang=this._objlangs[obj];if(this._handlers[lang]){output=this._handlers[lang].call(rs,obj,args);}else{output="[ERR: No Object Handler]";}}else{output="[ERR: Object Not Found]";}
...
return words.join(" ");}
return string;};RiveScript.prototype._strip_nasties=function(string){string=string.replace(/[^A-Za-z0-9 ]/g,"");return string;};RiveScript.prototype._escape_html=function(string){string=string.replace(/&/g,"&amp;");string=string.replace(/</g,"&lt;");string=string.replace(/>/g,"&gt;");string=string.replace(/"/g,"&quot;");return string;};publish(RiveScript);})((typeof(module)=="undefined"&&(typeof(window)!="undefined"&&this==window))?function(a){this["RiveScript"]=a;}:function(a){module.exports=a;});