...
//###############################################################################
export function test_load_directory_recursively(test) {
  let bot;
  bot = new TestCase(test, `
        + *
        - No, this failed.
	`);
  return bot.rs.loadDirectory('./test/fixtures').then(async() => {
    bot.rs.sortReplies();
...
export async function test_default_error_messages(test) {
  let DEF_NOT_FOUND;
  let DEF_NOT_MATCH;
  let DEF_NO_OBJECT;
  let DEF_RECURSION;
...
export async function test_error_constructor_configuration(test) {
  let bot;
  bot = new TestCase(test, `
        + condition only
        * <get name> == Aiden => Your name is Aiden!
...
export async function test_redirect_with_undefined_input(test) {
  let bot;
  // <@> test
  bot = new TestCase(test, `
        + test
...
export async function test_initialmatch(test) {
  let bot;
  bot = new TestCase(test, `
...
export async function test_valid_history(test) {
  let bot;
  bot = new TestCase(test, `
        + hello
        - Hi!
