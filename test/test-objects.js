import TestCase from "./test-base";

//###############################################################################
// Object Macro Tests
//###############################################################################
export async function test_js_objects(test) {
  let bot;
...
export async function test_disabled_js_language(test) {
  let bot;
  bot = new TestCase(test, `
...
export async function test_get_variable(test) {
  let bot;
  bot = new TestCase(test, `
		! var test_var = test
...
export async function test_uppercase_call(test) {
  let bot;
  bot = new TestCase(test, `
		> begin
			+ request
...
export async function test_objects_in_conditions(test) {
  let bot;
  bot = new TestCase(test, `
  ...
  return bot.rs.reply(bot.username, "call async 1").then(reply => {
    test.equal(reply, "Result: true");
    return bot.rs.reply(bot.username, "test async 1").then(reply => {
...
export async function test_line_breaks_in_call(test) {
  let bot;
  bot = new TestCase(test, `
...
export async function test_js_string_in_setSubroutine(test) {
  let bot;
  let input;
  bot = new TestCase(test, `
		+ hello
...
export async function test_function_in_setSubroutine(test) {
  let bot;
  let input;
  bot = new TestCase(test, `
		+ my name is *
...
export async function test_function_in_setSubroutine_return_value(test) {
  let bot;
  bot = new TestCase(test, `
		+ hello
		- hello <call>helper <star></call>
	`);
  bot.rs.setSubroutine("helper", (rs, args) => "person");
  await bot.reply("hello", "hello person");
  return test.done();
}

export async function test_arguments_in_setSubroutine(test) {
  let bot;
  bot = new TestCase(test, `
		+ my name is *
...
export async function test_quoted_strings_arguments_in_setSubroutine(test) {
  let bot;
  bot = new TestCase(test, `
		+ my name is *
...
export async function test_arguments_with_funky_spacing_in_setSubroutine(test) {
  let bot;
  bot = new TestCase(test, `
		+ my name is *
		- hello <call> helper "<star>"   12   "another  param" </call>
...
export function test_promises_in_objects(test) {
  let bot;
  let input;
  bot = new TestCase(test, `
		+ my name is *
	...
	`);
  input = "my name is Rive";
  bot.rs.setSubroutine("helperWithPromise", ({
    Promise
  }, args) => new Promise((resolve, reject) => resolve("stranger")));
  bot.rs.setSubroutine("anotherHelperWithPromise", ({
    Promise
  }) => new Promise((resolve, reject) => setTimeout(() => resolve("delay"), 10)));
  return bot.rs.reply(bot.username, input).then(reply => {
    test.equal(reply, "hello there stranger with a delay");
    return test.done();
  });
}

export async function test_use_reply_with_async_subroutines(test) {
  let bot;
  bot = new TestCase(test, `
		+ my name is *
		- hello there <call>asyncHelper</call>
	`);
  bot.rs.setSubroutine("asyncHelper", ({
    Promise
  }, args) => new Promise((resolve, reject) => resolve("stranger")));
...
export function test_async_and_sync_subroutines_together(test) {
  let bot;
  bot = new TestCase(test, `
		+ my name is *
  ...
  return bot.rs.reply(bot.username, "my name is Rive").then(reply => {
    test.equal(reply, "hello there stranger!");
    return test.done();
  });
}

export function test_stringify_with_objects(test) {
  let bot;
  let expect;
  let src;
  bot = new TestCase(test, `
...
export async function test_await_macro(test) {
