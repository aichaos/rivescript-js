 ...
 *
 * The methods prefixed with the word "private" *should not be used* by you.
 * They are documented here to help the RiveScript library developers understand
 * the code; they are not considered 'stable' API functions and they may change
 * or be removed at any time, for any reason, and with no advance notice.
 *
...
import InheritUtils from "./inheritance"
import JSObjectHandler from "./lang/javascript"
import Parser from "./parser"
import RSVP from "rsvp" //eslint-disable-line
 ...
 *#
 */
// Uncomment latter this.prototype.Promise = RSVP.Promise;
...
if (typeof opts.errors === "object") {
  ...
  for (const key in opts.errors) {

    const value = opts.errors[key];
...
this._global = {}; // 'global' variables
this._var = {}; // 'bot' variables
this._sub = {}; // 'sub' substitutions
this._submax = 1; // 'submax' max words in sub object
this._person = {}; // 'person' substitutions
this._personmax = 1; // 'personmax' max words in person object
this._array = {}; // 'array' variables
this._users = {}; // 'user' variables
this._freeze = {}; // Frozen 'user' variables
this._includes = {}; // Included topics
this._inherits = {}; // Inherited topics
this._handlers = {}; // Object handlers
this._objlangs = {}; // Map objects to their languages
this._topics = {}; // Main reply structure
this._thats = {}; // %Previous reply structure (pointers into @_topics)
...
const version = async() => VERSION,
   ...
   * #
   */
  warn = async(message, filename, lineno) => {
   ...
   * #
   */
  loadFile = async(path, onSuccess, onError) => {
  ...
  _ajaxLoadFile = async(loadCount, file, onSuccess, onError) => {
    ...
    xhr.onreadystatechange = () => {
      ...
      if (xhr.readyState === 4) {
        ...
        if ([
          ...
          this.say(`Loading file ${file} complete.`);

          // Parse it!
          this.parse(file, xhr.responseText, onError);

          // Log that we've received this file.
          delete this._pending[loadCount][file];

          // All gone?
          if (Object.keys(this._pending[loadCount]).length === 0) {

            if (typeof onSuccess === "function") {

              return onSuccess.call(undefined, loadCount);
          ...
          this.say(`Ajax error! ${xhr.statusText}; ${xhr.status}`);
          if (typeof onError === "function") {

            return onError.call(undefined, xhr.statusText, loadCount);
  ...
  _nodeLoadFile = async(loadCount, file, onSuccess, onError) => this._node.fs.readFile(file, (err, data) => {
   ...
   * #
   */
  loadDirectory = async(path, onSuccess, onError) => {
    ...
    return this._node.fs.stat(path, (err, stats) => {
      ...
      for (var file of files) {
        ...
        if (file.match(/\.(rive|rs)$/i)) {
          ...
          this._pending[loadCount][`${path}/${file}`] = 1;
      ...
      return (() => {
        ...
        for (file of toLoad) {
          ...
          this.say(`Parsing file ${file} from directory`);
          result.push(this._nodeLoadFile(loadCount, file, onSuccess, onError));
  ...
  parse = (filename, code, onError) => {
    ...
    const ast = this.parser.parse(filename, code, onError);

    // Get all of the "begin" type variables: global, var, sub, person, array..
    for (const type in ast.begin) {

      const vars = ast.begin[type];
      ...
      const internal = `_${type}`; // So "global" maps to this._global
      ...
      if (this._global.debug != null) {
        ...
        this._debug = this._global.debug === "true";
    ...
    for (const topic in ast.topics) {

      const data = ast.topics[topic];
    ...
    return (() => {
      ...
      for (const object of ast.objects) {
        ...
        if (this._handlers[object.language]) {
          ...
          result.push(this._handlers[object.language].load(object.name, object.code));
  ...
  sortReplies = async() => {
    ...
    for (const topic in this._topics) {
       ...
       * List as well.
       */
      const allTriggers = InheritUtils.getTopicTriggers(this, topic);

      // Sort these triggers.
      this._sorted.topics[topic] = sorting.sortTriggerSet(allTriggers, true);

      // Get all of the %Previous triggers for this topic.
      const thatTriggers = InheritUtils.getTopicTriggers(this, topic, true);
   ...
   * #
   */
  stringify = (deparsed) => this.parser.stringify(deparsed),
    ...
    return (() => {
      ...
      for (const key in data) {
        ...
        if (data[key] === undefined) {
          ...
          result.push(delete this._users[user][key]);

        } else {

          result.push(this._users[user][key] = data[key]);
  ...
  getUserTopicTriggers = (user) => {
    ...
    if (this.userVars != null) {
      ...
      return InheritUtils.getTopicTriggers(this, this.userVars.topic);
  ...
  currentUser = () => {
    ...
    if (this.brain._currentUser === undefined) {
   ...
   * #
   */
  reply = (user, msg, scope) => this.brain.reply(user, msg, scope);
 ...
 *   const reply = this.brain.reply(user, msg, scope, true);
 *   if (callback) {
 *     reply.then(result => {
