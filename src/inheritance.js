...
//#
const getTopicTriggers = (rs, topic, thats, depth, inheritance, inherited) => {
  ...
  if (!thats) {
    ...
    if (rs._topics[topic] != null) {
      for (trigger of Array.from(rs._topics[topic])) {
    ...
    if (rs._thats[topic] != null) {
      for (let curTrig in rs._thats[topic]) {
        if (!rs._thats[topic].hasOwnProperty(curTrig)) { continue; }
        for (let previous in rs._thats[topic][curTrig]) {
          if (!rs._thats[topic][curTrig].hasOwnProperty(previous)) { continue; }
          const pointer = rs._thats[topic][curTrig][previous];
  ...
  if (Object.keys(rs._includes[topic]).length > 0) {
    ...
    for (let includes in rs._includes[topic]) {
      if (!rs._includes[topic].hasOwnProperty(includes)) { continue; }
      rs.say(`Topic ${topic} includes ${includes}`);
      triggers.push(...getTopicTriggers(
  ...
  if (Object.keys(rs._inherits[topic]).length > 0) {
    ...
    for (let inherits in rs._inherits[topic]) {
      if (!rs._inherits[topic].hasOwnProperty(inherits)) { continue; }
  ...
  if ((Object.keys(rs._inherits[topic]).length > 0) || inherited) {
    for (trigger of Array.from(inThisTopic)) {
...
const getTopicTree = (rs, topic, depth) => {
  // Default depth
  if ((depth == null)) {
    depth = 0;
  }
  ...
  for (let includes in rs._topics[topic].includes) {
    if (!rs._topics[topic].includes.hasOwnProperty(includes)) { continue; }
    topics.push(...getTopicTree(rs, includes, depth+1));
  }

  for (let inherits in rs._topics[topic].inherits) {
    if (!rs._topics[topic].inherits.hasOwnProperty(inherits)) { continue; }
    topics.push(...getTopicTree(rs, inherits, depth+1));
  }

  return topics;
};

export {getTopicTriggers};
export {getTopicTree};
