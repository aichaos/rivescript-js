...
return reply;};var VERSION="1.0.5";var RS_VERSION="2.0";function RiveScript(opts){this._debug=false;this._strict=true;this._depth=50;this._utf8=false;this._div=undefined;this._node={};this._runtime=this.runtime();this._pending=[];this._loadcount=0;this._gvars={};this._bvars={};this._subs={};this._person={};this._arrays={};this._users={};this._freeze={};this._includes={};this._lineage={};this._handlers={};this._objlangs={};this._topics={};this._thats={};this._sorted={};this._current_user=undefined;if(typeof(opts)=="object"){if(opts["debug"]){this._debug=opts["debug"]?true:false;}
...
if(opts["utf8"]){this._utf8=!!opts["utf8"];}}
...
return;}
var to_load=[];for(var i=0,iend=files.length;i<iend;i++){if(files[i].match(/\.(rive|rs)$/i)){RS._pending[loadcount][path+"/"+files[i]]=1;to_load.push(path+"/"+files[i]);}}
...
this.say("Cmd: '"+cmd+"'; line: "+line);switch(cmd){case'!':var halves=line.split("=",2);var left=this._strip(halves[0]).split(" ");var value=type=name='';if(halves.length==2){value=this._strip(halves[1]);}
...
ontrig='';if(lang==undefined){this.warn("Trying to parse unknown programming language",fname,lineno);lang='javascript';}
...
if(this._users[user][name]){return this._users[user][name];}else{return"undefined";}};RiveScript.prototype.getUservars=function(user){if(user==undefined){return this._clone(this._users);}else{if(this._users[user]){return this._clone(this._users[user]);}else{return undefined;}}};RiveScript.prototype.clearUservars=function(user){if(user==undefined){this._users={};}else{delete this._users[user];}};RiveScript.prototype.freezeUservars=function(user){if(this._users[user]){this._freeze[user]=this._clone(this._users[user]);}else{this.warn("Can't freeze vars for user "+user+": not found!");}};RiveScript.prototype.thawUservars=function(user,action){if(typeof(action)!="string"){action="thaw";}
if(!this._freeze[user]){this.warn("Can't thaw user vars: "+user+" not found!");return;}
if(action=="thaw"){this.clearUservars(user);this._users[user]=this._clone(this._freeze[user]);delete this._freeze[user];}else if(action=="discard"){delete this._freeze[user];}else if(action=="keep"){this.clearUservars(user);this._users[user]=this._clone(this._freeze[user]);}else{this.warn("Unsupported thaw action");}};RiveScript.prototype.lastMatch=function(user){if(this._users[user]){return this._users[user]["__lastmatch__"];}
return undefined;};RiveScript.prototype.currentUser=function(){if(this._current_user==undefined){this.warn("currentUser() is intended to be called from within a JS object macro!");}
return this._current_user;};RiveScript.prototype.reply=function(user,msg,scope){this.say("Asked to reply to ["+user+"] "+msg);this._current_user=user;msg=this._format_message(msg);var reply='';if(this._topics["__begin__"]){var begin=this._getreply(user,"request","begin",0,scope);if(begin.indexOf("{ok}")>-1){reply=this._getreply(user,msg,"normal",0,scope);begin=begin.replace(/\{ok\}/g,reply);}
reply=begin;reply=this._process_tags(user,msg,reply,[],[],0,scope);}else{reply=this._getreply(user,msg,"normal",0,scope);}
this._users[user]["__history__"]["input"].pop();this._users[user]["__history__"]["input"].unshift(msg);this._users[user]["__history__"]["reply"].pop();this._users[user]["__history__"]["reply"].unshift(reply);this._current_user=undefined;return reply;};RiveScript.prototype._format_message=function(msg,botreply){msg=""+msg;msg=msg.toLowerCase();msg=this._substitute(msg,"subs");if(this._utf8){msg=msg.replace(/[\\<>]+/,"");if(botreply){msg=msg.replace(/[.?,!;:@#$%^&*()]/,"");}}
else{msg=this._strip_nasties(msg);}
return msg;};RiveScript.prototype._getreply=function(user,msg,context,step,scope){if(!this._sorted["topics"]){this.warn("You forgot to call sortReplies()!");return"ERR: Replies Not Sorted";}
...
for(var i=0,iend=allTopics.length;i<iend;i++){var top=allTopics[i];this.say("Checking topic "+top+" for any %Previous's.");if(this._sorted["thats"][top]){this.say("There's a %Previous in this topic!");var lastReply=this._users[user]["__history__"]["reply"][0];lastReply=this._format_message(lastReply,true);this.say("Last reply: "+lastReply);for(var j=0,jend=this._sorted["thats"][top].length;j<jend;j++){var trig=this._sorted["thats"][top][j];var botside=this._reply_regexp(user,trig);this.say("Try to match lastReply ("+lastReply+") to "+botside);var match=lastReply.match(new RegExp('^'+botside+'$'));if(match){this.say("Bot side matched!");thatstars=[];for(var k=1,kend=match.length;k<kend;k++){thatstars.push(match[k]);}
...
regexp=regexp.replace(/\\w/,"[A-Za-z]");var giveup=0;while(regexp.indexOf("@")>-1){giveup++;if(giveup>=50){break;}
...
if(regexp.indexOf("<input")>-1||regexp.indexOf("<reply")>-1){regexp=regexp.replace(/<input>/i,"<input1>");regexp=regexp.replace(/<reply>/i,"<reply1>");var types=["input","reply"];for(var i=0;i<2;i++){var type=types[i];for(var j=1;j<=9;j++){if(regexp.indexOf("<"+type+j+">")){regexp=regexp.replace(new RegExp("<"+type+j+">","g"),this._users[user]["__history__"][type][j]);}}
...
reply=reply.replace(/<call>/ig,"{__call__}");reply=reply.replace(/<\/call>/ig,"{/__call__}");while(true){match=reply.match(/<([^<]+?)>/);if(!match)
break;match=match[1];var parts=match.split(" ",2);var tag=parts[0].toLowerCase();var data="";if(parts.length>1){data=parts[1];}
var insert="";if(tag==="bot"||tag==="env"){var target=tag==="bot"?this._bvars:this._gvars;if(data.indexOf("=")>-1){var parts=data.split("=",2);this.say("Set "+tag+" variable "+parts[0]+" = "+parts[1]);target[parts[0]]=parts[1];}
else{insert=target[data]||"undefined";}}
else if(tag==="set"){var parts=data.split("=",2);this.say("Set uservar "+parts[0]+" = "+parts[1]);this._users[user][parts[0]]=parts[1];}
else if(tag==="add"||tag==="sub"||tag==="mult"||tag==="div"){var parts=data.split("=");var name=parts[0];var value=parts[1];if(!this._users[user][name]){this._users[user][name]=0;}
value=parseInt(value);if(isNaN(value)){insert="[ERR: Math can't '"+tag+"' non-numeric value '"+value+"']";}else if(isNaN(parseInt(this._users[user][name]))){insert="[ERR: Math can't '"+tag+"' non-numeric user variable '"+name+"']";}else{var orig=parseInt(this._users[user][name]);var newval=0;if(tag==="add"){newval=orig+value;}
else if(tag==="sub"){newval=orig-value;}
else if(tag==="mult"){newval=orig*value;}
else if(tag==="div"){if(value===0){insert="[ERR: Can't Divide By Zero]"}else{newval=orig/value;}}
if(insert===""){this._users[user][name]=newval;}}}
else if(tag==="get"){insert=this._users[user][data]||"undefined";}
else{insert="\x00"+match+"\x01";}
...
reply=reply.replace(/\{__call__\}/g,"<call>");reply=reply.replace(/\{\/__call__\}/g,"</call>");match=reply.match(/<call>(.+?)<\/call>/i);giveup=0;while(match){giveup++;if(giveup>=50){this.warn("Infinite loop looking for call tag!");break;}
...
var ph=[];var pi=0;for(var i=0,end=this._sorted["lists"][list].length;i<end;i++){var pattern=this._sorted["lists"][list][i];var result=subs[pattern];var qm=this.quotemeta(pattern);ph.push(result);var placeholder="\x00"+pi+"\x00";pi++;msg=msg.replace(new RegExp("^"+qm+"$","g"),placeholder);msg=msg.replace(new RegExp("^"+qm+"(\\W+)","g"),placeholder+"$1");msg=msg.replace(new RegExp("(\\W+)"+qm+"(\\W+)","g"),"$1"+placeholder+"$2");msg=msg.replace(new RegExp("(\\W+)"+qm+"$","g"),"$1"+placeholder);}
var tries=0;while(msg.indexOf("\x00")>-1){tries++;if(tries>50){this.warn("Too many loops in substitution placeholders!");break;}
var match=msg.match("\\x00(.+?)\\x00");if(match){var cap=parseInt(match[1]);var result=ph[cap];msg=msg.replace(new RegExp("\x00"+cap+"\x00","g"),result);}}
...
return words.join(" ");}
return string;};RiveScript.prototype._strip_nasties=function(string){if(this._utf8){string=string.replace(/[\\<>]+/g,"");return string;}
string=string.replace(/[^A-Za-z0-9 ]/g,"");return string;};RiveScript.prototype._escape_html=function(string){string=string.replace(/&/g,"&amp;");string=string.replace(/</g,"&lt;");string=string.replace(/>/g,"&gt;");string=string.replace(/"/g,"&quot;");return string;};RiveScript.prototype._clone=function(obj){if(obj===null||typeof(obj)!="object"){return obj;}
var copy=obj.constructor();for(var key in obj){copy[key]=this._clone(obj[key]);}
return copy;};RiveScript.prototype._shim_keys=function(){if(!Object.keys){Object.keys=(function(){var hasOwnProperty=Object.prototype.hasOwnProperty,hasDontEnumBug=!({toString:null}).propertyIsEnumerable('toString'),dontEnums=['toString','toLocaleString','valueOf','hasOwnProperty','isPrototypeOf','propertyIsEnumerable','constructor'],dontEnumsLength=dontEnums.length;return function(obj){if(typeof(obj)!=='object'&&typeof(obj)!=='function'||obj==null)throw new TypeError('Object.keys called on non-object');var result=[];for(var prop in obj){if(hasOwnProperty.call(obj,prop)){result.push(prop);}}
